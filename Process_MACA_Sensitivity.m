...
% DATE:   27 Jan. 2015
% DESC:   This script accesses MACAv2-METDATA to derive GU, LSF, and FSEI.
...
%-----------------------------------------------------------------------------
disp('Preallocating Model_Sensitivity_Mean.mat file...')
mdl_senstvty_file = matfile('Model_Sensitivity_Mean.mat','Writable',true);
...
%%============================================================================
% Load and create necessary data/constants.
%-----------------------------------------------------------------------------

% Load tmin deltas for sensitivity experiment.
disp('Loading MACA tmin delta...')
load('MACA_Sensitivity_Deltas.mat','delta_mean');

% Open day_length data.
disp('Loading MACA daylength...')
...
disp('Setting daylength and VPD varibles...')
HOUR_LOW = 10;
HOUR_HIGH = 11;
day_length = day_length(DAY_IND,:);
...
%%============================================================================
% Main loop to access MACA files and derive events.
%-----------------------------------------------------------------------------
...
for mdl=1:N_MDL
	...
	%---------------------------------------------------------------------
    % Enter LON and LAT loops. Subset x/y and preallocate variables.
    %---------------------------------------------------------------------
	...
	for x=1:length(LON_START)
	    ...
	    for y=1:length(LAT_START)
			...
			%-----------------------------------------------------------------
			% Load temperature data into memory.
			%-----------------------------------------------------------------
            ...
            t_delta = squeeze(delta_mean(lat_subset,lon_subset,mdl));
