...
lsf_file = matfile('/home/alex/LSF_METDATA.mat','Writable',true);
gu_file = matfile('/home/alex/GU_METDATA.mat','Writable',true);

% Preallocate space with NaNs.
lsf_file.data = NaN(N_YRS,N_LAT,N_LON,'single');
gu_file.data = NaN(N_YRS,N_LAT,N_LON,'single');
...
for x=1:length(LON_START)
    ...
    for y=1:length(LAT_START)
        ...
        lsf_file.data(1:30,lat_subset,lon_subset) = lsf_subset;
        gu_file.data(1:30,lat_subset,lon_subset) = gu_subset;
