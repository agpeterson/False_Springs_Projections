...
%%=============================================================================
% Pull one model to calculate day length (photoperiod) and save for future
% calls.
%==============================================================================
...
day_length=day_length';     % Transpose such that lat is outside dimension.
...
m_new = matfile('false_springs.mat','Writable',true);
    ...
    % for e=1:3  % REFACTOR - should not be hard-coded.
        ...
        for x=1:length(LON_START)
            ...
            for y=1:length(LAT_START)
                ...
                t_var = file.data(:,:,lat_subset,lon_subset);
                ...
                lsf_subset = NaN(n_yrs,length(lat_subset),length(lon_subset));
                gsi_subset = NaN(n_yrs,length(lat_subset),length(lon_subset));
                
                tic
                % Parallel iteration over lon_subset.
                parfor i=1:length(lon_subset)

                    % Create temporary variable for each lon_subset.
                    t_var2 = t_var(:,:,:,i);

                    % Call parallel function.
                    [lsf_sub,gsi_sub] = parallel_function(t_var2(:,:,:),...
                                                          day_subset(:,:),...
                                                          vpd_subset(:,:),...
                                                          n_yrs(:),...
                                                          lat_subset(:,:));   
                    
                    % Concatenate subsets to respective variable.
                ...
                %m_new.lsf_CONUS(yr_index,lat_subset,lon_subset,m) = single(lsf_subset);
                %m_new.gsi_CONUS(yr_index,lat_subset,lon_subset,m) = single(gsi_subset);
